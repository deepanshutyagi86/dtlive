...
export default function Doors({ counts }: { counts: Record<string, number> }) {
  return (
    <section className="max-w-[1200px] mx-auto px-5 pt-20 pb-10">
      <p className="font-mono text-[11px] tracking-[0.14em] uppercase text-muted mb-2">Pick a door</p>
      {DOORS.map((d, i) => {
        ...
        return (
          ...
          >
            ...
            <div className="relative z-[2] mt-1.5 text-sm text-muted max-h-0 opacity-0 overflow-hidden transition-all duration-300 group-hover:max-h-11 group-hover:opacity-100 group-hover:text-[#b9b8ae] hidden md:block">
              {d.sub}
            </div>
          </Link>
        );
