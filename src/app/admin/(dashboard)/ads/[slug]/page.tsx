...
import { getAllAdPages } from "@/lib/site-settings";
...
export default async function AdPageSignupsPage({ params }: { params: { slug: string } }) {
  // Deliberately every page, not adPageFor's on-only lookup: a finished
  // campaign is usually switched off, and that is exactly when you most
  // want to see what it produced.
  const pages = await getAllAdPages();
  const page = pages.find((p) => p.slug === params.slug) ?? null;

  const [rows, columnReady] = await Promise.all([
    listBySource(adSourceTag(params.slug)),
    hasSourceColumn(),
  ]);
  ...
  return (
    <div>
      <Link href="/admin/ads" className="font-mono text-[11px] text-muted hover:text-ink transition-colors">
        ← Ad pages
      </Link>
      <h1 className="font-display font-extrabold text-3xl tracking-tight mt-2">
        {page?.headline || params.slug}
      </h1>
      <p className="text-[13px] text-muted mt-1">
        Everyone who registered or bought from /w/{params.slug}.
      </p>
      ...
      {!columnReady ? (
        <div className="mt-8 border border-marigold bg-marigold/10 rounded-card px-5 py-4">
          ...
          <p className="text-[14px] text-ink-soft mt-1.5 leading-relaxed">
            Registrations and payments from /w are being saved normally — but which ad page they came from
